...
#include "JeVe_EasyOTA_V2.h"  // https://github.com/jeroenvermeulen/JeVe_EasyOTA/blob/master/src/JeVe_EasyOTA.h
...
void EasyOTA::setup(const char* wifi_ssid, const char* wifi_password, const char* hostname) {
  showMessage(const_cast<char*>(""), 1); // New line in case of using serial output
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    showMessage(const_cast<char*>("Can't connect WiFi"), 1);
    showMessage(const_cast<char*>("Going into AP mode."), 2);
    WiFi.mode(WIFI_AP);
  ...
  ArduinoOTA.onStart([this]() {
    showMessage(const_cast<char*>("OTA starting..."), 2);
  });
  ArduinoOTA.onEnd([this]() {
    showMessage(const_cast<char*>("OTA done.Reboot..."),2);
